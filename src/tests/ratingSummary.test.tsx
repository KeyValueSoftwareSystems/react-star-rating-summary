...
describe('RatingSummary', () => {
    ...
    it('should not display the rating count for each rating ID when showCount is false', () => {
        const { container } = render(<RatingSummary ratings={ratings} showCount={false} />);
        const oneStarRatingCount = getById(container, 'one-count');
        const twoStarRatingCount = getById(container, 'two-count');
        const threeStarRatingCount = getById(container, 'three-count');
        const fourStarRatingCount = getById(container, 'four-count');
    ...
    it('should display the chart with correct width based on the number of ratings', () => {
        ...
        expect(oneStarBar.style._values.width).toBe(`${ratings.one / total * 100}%`);
        expect(twoStarBar.style._values.width).toBe(`${ratings.two / total * 100}%`);
        expect(threeStarBar.style._values.width).toBe(`${ratings.three / total * 100}%`);
        expect(fourStarBar.style._values.width).toBe(`${ratings.four / total * 100}%`);
        expect(fiveStarBar.style._values.width).toBe(`${ratings.five / total * 100}%`);
    });
});